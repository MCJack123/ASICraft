...
public class SHA3TransformAlgorithm implements IAlgorithm {
    MessageDigest hasher = null;

    @Override
    public String getType() {
        return "transform_sha3";
    }
    ...
    public void input(IArguments args, IAlgorithmPartialResultCallback callback) throws LuaException {
        if (hasher == null) {
            try {hasher = MessageDigest.getInstance("SHA3-256");}
            catch (NoSuchAlgorithmException ignored) {throw new LuaException("Could not create hasher: Algorithm not available");}
        }
